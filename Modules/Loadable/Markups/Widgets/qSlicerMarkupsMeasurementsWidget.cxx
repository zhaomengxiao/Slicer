...
class qSlicerMarkupsMeasurementsWidgetPrivate
  : public Ui_qSlicerMarkupsMeasurementsWidget
{
...
void qSlicerMarkupsMeasurementsWidgetPrivate::setupUi(qSlicerMarkupsMeasurementsWidget* widget)
{
  ...
  QObject::connect(this->measurementsListView, SIGNAL(currentItemChanged(vtkIdType)),
  ...
  QObject::connect(this->measurementsListView, SIGNAL(customContextMenuRequested(QPoint)),
                   q, SLOT(onRightClickActiveMeasurement(QPoint)));

  QObject::connect(this->removeMeasurementPushButton, SIGNAL(clicked()),
    q, SLOT(onDeleteMeasurement()));

  QObject::connect(this->goToMarkupsModulePushButton, SIGNAL(clicked()),
    q, SLOT(onGoToMarkupsModule()));
...
void qSlicerMarkupsMeasurementsWidgetPrivate::setAllowedMarkups(const QStringList& allowedMarkups)
{
  ...
  foreach(const std::string &markupName, q->markupsLogic()->GetRegisteredMarkupsTypes())
    ...
    if (className.compare("vtkMRMLMarkupsFiducialNode") == 0)
      {
      continue;
      }
...
void qSlicerMarkupsMeasurementsWidgetPrivate::createMarkupsPushButtons()
{
  ...
  foreach(const std::string &markupName, q->markupsLogic()->GetRegisteredMarkupsTypes())
    ...
    markupCreatePushButton->setObjectName(QString(q->tr("Create")) +
                                          QString(markupsNode->GetMarkupType()) +
                                          QString(q->tr("PushButton")));
    markupCreatePushButton->setIcon(QIcon(markupsNode->GetPlaceAddIcon()));
    markupCreatePushButton->setToolTip(QString(q->tr("Create ")) +
                                       QString(markupsNode->GetTypeDisplayName()));
    layout->addWidget(markupCreatePushButton,
                      markupIndex / this->createMarkupsButtonsColumns,
                      markupIndex % this->createMarkupsButtonsColumns);
...
vtkMRMLInteractionNode* qSlicerMarkupsMeasurementsWidgetPrivate::interactionNode()
{
  ...
  vtkMRMLInteractionNode* interactionNode =
    vtkMRMLInteractionNode::SafeDownCast(q->mrmlScene()->GetNodeByID("vtkMRMLInteractionNodeSingleton"));
...
//-----------------------------------------------------------------------------
void qSlicerMarkupsMeasurementsWidgetPrivate::setPlaceModeEnabled(bool placeEnable)
{
  Q_Q(qSlicerMarkupsMeasurementsWidget);
...
void qSlicerMarkupsMeasurementsWidgetPrivate::jumpAllSlices(vtkMRMLSliceNode* exclude /* =nullptr */)
{
  ...
  vtkMRMLSliceNode::JumpAllSlices(q->mrmlScene(), boundsCenter[0], boundsCenter[1], boundsCenter[2],
    vtkMRMLSliceNode::OffsetJumpSlice, this->ViewGroup, exclude);
}
...
qSlicerMarkupsMeasurementsWidget::qSlicerMarkupsMeasurementsWidget(QWidget* parentWidget) :
  Superclass( parentWidget ) , d_ptr( new qSlicerMarkupsMeasurementsWidgetPrivate(*this) )
{
  this->setup();
}
...
qSlicerMarkupsMeasurementsWidget::~qSlicerMarkupsMeasurementsWidget()
{
  ...
  if (d->CurrentMarkupsNode)
    ...
    qvtkDisconnect(d->CurrentMarkupsNode, vtkMRMLMarkupsNode::MeasurementsCollectionModifiedEvent,
...
void qSlicerMarkupsMeasurementsWidget::setCurrentNode(vtkMRMLNode* currentNode)
{
  ...
  if (currentMarkupsNode)
    {
    qvtkConnect(currentMarkupsNode, vtkMRMLMarkupsNode::MeasurementsCollectionModifiedEvent,
      this, SLOT(onMeasurementsCollectionModified()));
    }
...
void qSlicerMarkupsMeasurementsWidget::onEndPlacementEvent()
{
  ...
  if (isAMeasurement.compare("true") != 0)
    {
    return;
    }
...
void qSlicerMarkupsMeasurementsWidget::onRightClickActiveMeasurement(QPoint pos)
{
  ...
  // Rename
  QAction *renameMarkupsAction =
    new QAction(QString("Rename"), &menu);
  menu.addAction(renameMarkupsAction);
  QObject::connect(renameMarkupsAction, SIGNAL(triggered()),
                   this, SLOT(onRenameMeasurement()));

  // Clone
  QAction *cloneMarkupsAction =
    new QAction(QString("Clone"), &menu);
  menu.addAction(cloneMarkupsAction);
  QObject::connect(cloneMarkupsAction, SIGNAL(triggered()),
                   this, SLOT(onCloneMeasurement()));

  // Delete
  QAction *deleteMarkupsAction =
    new QAction(QString("Delete"), &menu);
  menu.addAction(deleteMarkupsAction);
  QObject::connect(deleteMarkupsAction, SIGNAL(triggered()),
                   this, SLOT(onDeleteMeasurement()));

  menu.addSeparator();

  // Advanced Options
  QAction *advancedOptionsMarkupsAction =
    new QAction(QString("Advanced Settings"), &menu);
  menu.addAction(advancedOptionsMarkupsAction);
  QObject::connect(advancedOptionsMarkupsAction, SIGNAL(triggered()),
                   this, SLOT(onGoToMarkupsModule()));
...
void qSlicerMarkupsMeasurementsWidget::populateMeasurementSettingsTable()
{
  ...
  d->measurementSettingsTableWidget->setVisible(
    d->CurrentMarkupsNode != nullptr && d->CurrentMarkupsNode->GetNumberOfMeasurements() > 0 );
...
void qSlicerMarkupsMeasurementsWidget::setMRMLScene(vtkMRMLScene* scene, const QStringList& allowedMarkups)
{
  ...
  if (interactionNode)
    {
    this->qvtkConnect(interactionNode, vtkMRMLInteractionNode::EndPlacementEvent,
                   this, SLOT(onEndPlacementEvent()));
    }
...
void qSlicerMarkupsMeasurementsWidget::onCreateMarkupByClass(const QString& className)
{
  ...
  vtkMRMLMarkupsNode* defaultMarkupsNode = vtkMRMLMarkupsNode::SafeDownCast
    (this->mrmlScene()->GetDefaultNodeByClass(className.toStdString().c_str()));
  if (defaultMarkupsNode)
    ...
    if (defaultMeasurements->GetNumberOfValues() != 0)
      ...
      for (int index = 0; index < defaultMeasurements->GetNumberOfValues(); ++index)
        ...
        markupsNode->EnableMeasurement(measurementName);
        }
      }
    }
